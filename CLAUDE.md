...
HTML Vault: ユーザーがアップロードした静的ファイル (HTML/JS/CSS/JSON/SVG) を保存し、直リンク (`/v/:public_id`) でサンドボックス化されたiframe上にプレビュー表示できるツール。関連する複数ファイルは「バンドル」(`Bundle` has_many `UserFile`) としてまとめてアップロードでき、HTML内の相対パス参照 (`<link href="style.css">`) が正しく解決されるよう `/v/b/:bundle_public_id/*filename` で配信される。管理画面 (アップロード/一覧/削除) は `frontend/` 配下のReact SPAが担い、Railsはその API とファイル配信ルートを担当する。
...
bin/importmap audit                                                      # importmapの脆弱性監査 (importmap-railsはrails new標準の未使用ボイラープレートで、実際のフロントは使っていない)
...
- **単一のRailsアプリで、別建てのAPIサーバーは無い。** `Api::V1::*` コントローラーがJSONを返し、`PreviewController` がアップロード済みファイルの実体を直接配信し、それ以外は `FallbackController` がSPAのentry HTMLを返す。`vite_rails` gemは使わず、Rails/Vite統合は `app/helpers/application_helper.rb` に自前実装されている (詳細は `docs/frontend-architecture.md`)。
...
- **コンテンツを表すモデルは2つ、IDの仕組みは共通化**: `UserFile` (単体ファイル、`has_one_attached :file`) と `Bundle` (`has_many :user_files, dependent: :destroy`、関連ファイルのセット)。どちらも `HasPublicId` (`app/models/concerns/has_public_id.rb`) を `include` し、内部の連番 `id` を外部に晒さないための `public_id` (`SecureRandom.alphanumeric(8)`、DB側で重複チェック) を持つ。`UserFile#bundle_id` はnullableで、`nil` なら単体ファイルを意味し、`Api::V1::UserFilesController#index` はこの条件で絞り込む (バンドル配下のファイルは `Api::V1::BundlesController` 経由でのみ一覧取得できる)。
- **ファイル配信 (`PreviewController`)** はActive Storageのblobを `send_data` で直接ストリーミングする。`sandbox` CSP (意図的に `allow-same-origin` を含めない) でアップロードされた信頼できないHTML/JSを親アプリのCookie/セッションから隔離しており、加えて `skip_forgery_protection` を指定している — Railsの標準CSRF機構はクロスオリジンからのJavaScriptレスポンスへのGETをデフォルトでブロックする (レガシーなJSONP型攻撃を防ぐための仕組み) ため、これが無いとバンドル内の `<script src="app.js">` はおろか単体JSファイルのプレビューすら壊れる。
- **バンドルのルーティング**: `/v/b/:bundle_public_id/*filename` はグロブセグメントに `format: false` を指定している。指定しないと `.css` のような末尾の拡張子がRailsによってレスポンスフォーマットとして解釈・除去され、キャプチャされたファイル名が壊れてルックアップに失敗する。
